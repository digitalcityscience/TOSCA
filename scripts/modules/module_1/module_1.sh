...
# Message 1 Start points are required. Do you want to draw start points on the basemap now? If yes, click Yes. If you do not want to draw points, becuse you want to use an already existing ma, select No. To exit tis module click Cancel. 
    Send_Message 1 module_1.1
        Request
            case $REQUEST_CONTENT in
                ...
                "no"|"No"|"NO")
                    Send_Message 2 module_1.2  # Waiting for a map name (map already have to exist in GRASS)
                    Request
                        FRESH=$REQUEST_CONTENT
                        Add_Vector $FRESH from_points
    ...
    Send_Message 3 module_1.3
        Request
            case $REQUEST_CONTENT in
                ...
                "no"|"No"|"NO")
                    VIA=1
                    Send_Message 2 module_1.4  # Waiting for a map name (map already have to exist in GRASS)
                    Request
                        FRESH=$REQUEST_CONTENT
                        Add_Vector $FRESH via_points
                        Gpkg_Out via_points via_points
    ...
    Send_Message 4 module_1.5
        Request
            case $REQUEST_CONTENT in
                ...
                "no"|"No"|"NO")
                    TO=1
                    Send_Message 2 module_1.6  # Waiting for a map name (map already have to exist in GRASS)
                    Request
                        FRESH=$REQUEST_CONTENT
                        Add_Vector $FRESH to_points
                        Gpkg_Out to_points to_points
    ...
    Send_Message 5 module_1.7
        Request
            case $REQUEST_CONTENT in
                ...
                "no"|"No"|"NO")
                    AREA=1
                    Send_Message 6 module_1.8  # Waiting for a map name (map already have to exist in GRASS)
                    Request
                        FRESH=$REQUEST_CONTENT
                        Add_Vector $FRESH area
                        Gpkg_Out area area
...
# Messages 14 Do you want to s    echo $i"="$i >> $MODULES/module_1/reclass_rules_1et the speed on the road network? If not, the current values will used (km/h). If you want to change the values, you may overwrite those. Do not remove semicolons from the end of lines.
    Send_Message 7 module_1.9
        echo >> $MESSAGE_SENT/message.module_1.5
        cat $VARIABLES/roads_speed >> $MESSAGE_SENT/message.module_1.5
            ...
            case $REQUEST_CONTENT in
                "yes"|"Yes"|"YES")
                    Request
                        ...
                        SPECIFIC_VALUE=$(cat $VARIABLES/roads_speed | head -n$n | tail -n1 | cut -d":" -f2 | sed s'/ //'g);;
                "no"|"No"|"NO")
                    ;;
            esac
...
# Creating highways map. This is fundamental for the further work in this module
    ...
    case $AREA in
        0|1)
            # Clip roads map
                grass $GRASS/$MAPSET --exec v.overlay ainput=highways atype=line binput=$AREA_MAP operator=not output=highways_clipped --overwrite --quiet 
                grass $GRASS/$MAPSET --exec g.rename vector=highways_clipped,highways --overwrite --quiet;;
    esac
...
# To points are not optional. Optional only to place them on-by-one on the map, or  selecting an already existing map. If there are no user defined/selected to_points, default points (highway_points) are used as to_points. But, because these points are on the road by its origin, therefore no further connecting is requested.
    if [ $TO -eq 0 -o $TO -eq 1 ]
        then
            ...
            if [ $TO -eq 2 ]
                then
                    grass $GRASS/$MAPSET --exec v.to.points input=highways output=highway_points dmax=0.002 --overwrite --quiet
                    TO_POINT="highway_points"
            fi
    fi

# threshold to connect is ~ 220 m
grass $GRASS/$MAPSET --exec v.net input=highways points=from_via_to_points output=highways_points_connected operation=connect threshold=0.003 --overwrite --quiet
...
# Now the Supplementary lines (formerly CAT_SUPP_LINES) raster map have to be added to map highways_from_points. First I convert highways_points_connected into raster setting value to 0(zero). Resultant map: temp. After I patch temp and highways_points_connected, result is:highways_points_connected_temp. Now have to reclass highways_points_connected_temp, setting 0 values to the speed value of residentals
    grass $GRASS/$MAPSET --exec v.to.rast input=highways_points_connected output=temp use=val val=$SPECIFIC_VALUE --overwrite --quiet
    grass $GRASS/$MAPSET --exec r.patch input=highways_points_connected,temp output=highways_points_connected_full --overwrite --quiet
    grass $GRASS/$MAPSET --exec r.mapcalc expression="highways_points_connected_full=float(highways_points_connected_full)" --overwrite --quiet
...
# Calculating from -- via time map, via -- to time map and it sum. There is a NULL value replacenet too. It is neccessary, because otherwise, if one of the maps containes NULL value, NULL value cells will not considering while summarizing the maps. Therefore, before mapcalc operation, NULL has to be replaced by 0.
    if [ $VIA -eq 0 -o $VIA -eq 1 ]
        then
            ...
            grass $GRASS/$MAPSET --exec r.null map=from_via_cost null=0
